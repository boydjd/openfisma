...
#	exit;
	...
	# handle errors
		...
		foreach my $err_str_ref (@$err_str_arr_ref) {
			...
			if (ref($err_str_ref) eq 'ARRAY') { inject_utils::log_write($err_str_ref->[1], $err_str_ref->[0]); }

			else { inject_utils::log_write(2, "$err_str_ref (bare string - check reporting code)"); }
