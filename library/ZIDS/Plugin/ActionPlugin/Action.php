...
abstract class ZIDS_Plugin_ActionPlugin_Action implements ZIDS_Plugin_ActionPlugin_Interface 
{
	...
	final function getOptions($levelname) {
		...
		$global = (
			isset($this->_config['*']['option'][$this->getIdentifier()]) ? $this->_config['*']['option'][$this->getIdentifier()] : 
			(
				isset($this->_config['all']['option'][$this->getIdentifier()]) ? $this->_config['all']['option'][$this->getIdentifier()] : null
			)
		);
