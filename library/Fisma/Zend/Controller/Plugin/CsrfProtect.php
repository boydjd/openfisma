...
class Fisma_Zend_Controller_Plugin_CsrfProtect extends Zend_Controller_Plugin_Abstract
{
    ...
    public function preDispatch(Zend_Controller_Request_Abstract $request)
    {
        if ($request->isPost() && $this->getRequest()->getControllerName() != "error") {
            $invalidErrorMessage = 'A possible CSRF attack detected: tokens do not match.';
            if ($this->getRequest()->getControllerName() == "auth") {
                $appname = Fisma::configuration()->getConfig('system_name');
                $here = '<a href="/auth/login/">here</a>';
                $invalidErrorMessage = "
                    The security token generated by $appname has expired. This may be due to long periods of inactivity.
            ...
            if (!$this->isValidToken($value))
                throw new Fisma_Zend_Exception_User($invalidErrorMessage);
        }
    }
