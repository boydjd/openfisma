...
class Fisma_Zend_Controller_Action_Helper_FismaContextSwitch extends Zend_Controller_Action_Helper_ContextSwitch
{
    ...
    public function init()
    {
        ...
        // Fix a IE7 bug where JSON responses in an iframe are treated as attachments rather than inline.
        $agent = new Zend_Http_UserAgent;
        $device = $agent->getDevice();

        if (stristr($device->getBrowser(), 'explorer') && stristr($device->getBrowserVersion(), '7')) {
            $this->setContext(
                'json',
                array(
                    'suffix'    => 'json',
