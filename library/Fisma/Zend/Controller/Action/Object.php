...
abstract class Fisma_Zend_Controller_Action_Object extends Fisma_Zend_Controller_Action_Security
{
    ...
    public function _createObject()
    {
        ...
        if ($this->_request->isPost()) {
            ...
            if ($form->isValid($post)) {
                ...
                $this->view->priorityMessenger("Unable to create the "
                    . "{$this->getSingularModelName()}:<br>$errorString", 'warning');
            }
        }
    ...
    protected function _editObject()
    {
        ...
        if ($this->_request->isPost()) {
            ...
            if ($form->isValid($post)) {
                ...
                $errorString = Fisma_Zend_Form_Manager::getErrors($form);
                $error = "Error while trying to save: {$this->getSingularModelName()}: <br>$errorString";
    ...
    protected function _getSubject($id)
    {
        ...
        if (false == $record) {
             ...
             throw new Fisma_Zend_Exception_User(sprintf($msg, $this->getSingularModelName(), $id));
        }
