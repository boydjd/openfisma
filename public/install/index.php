...
case "dbform":
    ...
    $config = file_get_contents(OVMS_WEB_PATH . '/mainfile.php');
    $isTheFirstTime = preg_match('/OVMS_INSTALL/', $config);
    unset($config);
    $sm = new setting_manager('false'); 
    if(!$isTheFirstTime){
        include(OVMS_WEB_PATH . '/mainfile.php');
        $sm->readConstant();
     }
...
case "initial":
    include_once(OVMS_WEB_PATH.'/mainfile.php');
