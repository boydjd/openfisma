...
class Application_Migration_021700_BootstrapMigration extends Fisma_Migration_Abstract
{
    ...
    public function migrate()
    {
        ...
        $this->message("Creating migration table…");
        $this->getHelper()->createTable('migration', $columns, 'id');

        if ($this->getHelper()->tableExists('migration_version')) {
            $this->message("Dropping doctrine migration_version table…");
            $this->getHelper()->dropTable('migration_version');
        } else {
            $this->message("Doctrine migration_version table not found… skipping.");
        }
    }
}
