...
$worksheet->write($rowi,0,'Products with Open Vulnerabilities',$format_header);
...
$worksheet->write($rowi,3,'# of Open Vulnerabilities',$format_header);
$rowi++;
...
for($i=0;$i<count($this->rpdata);$i++){
    ...
    $worksheet->write($rowi+$i,3,$this->rpdata[$i]['NumoOV'],$format_times);
}
